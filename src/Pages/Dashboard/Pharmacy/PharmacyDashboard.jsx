...
function OrderRow({ order }) {
  const statusMap = { pending: "bg-amber-100 text-amber-700", delivered: "bg-green-100 text-green-700", cancalled: "bg-red-100 text-red-600" };
  ...
  return (
    <tr className="border-b border-gray-50 hover:bg-gray-50/60 transition-colors">
      <td className="py-3 px-4"><div className="flex items-center gap-2.5"><div className="w-7 h-7 rounded-lg bg-green-50 border border-green-100 flex items-center justify-center text-sm flex-shrink-0">💊</div><div><p className="text-[13px] font-bold text-gray-800">#{order._id?.slice(-8).toUpperCase()}</p><p className="text-[10px] text-gray-400">{new Date(order.createdAt).toLocaleDateString("en-NP", { day: "numeric", month: "short" })}</p></div></div></td>
      ...
      <div className="pl-[200px]">
        <main className="px-8 py-7 min-h-screen">
          <div className="mb-7 flex items-start justify-between">
            <div><h1 className="text-[26px] font-black text-gray-900 tracking-tight leading-tight">Pharmacy Dashboard</h1><p className="text-gray-400 text-[13px] mt-0.5">Real-time overview of your orders and revenue</p></div>
            <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 bg-gray-950 text-white px-4 py-2.5 rounded-xl font-bold text-[13px] hover:bg-gray-800 transition shadow-sm"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4"/></svg>Add Product</button>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <div><h2 className="text-[15px] font-black text-gray-900">Recent Orders</h2><p className="text-[11px] text-gray-400 mt-0.5">{recentOrders.length} most recent customer orders</p></div>
            ...
            {loading ? (
              ...
              <table className="w-full">
                <thead><tr className="border-b border-gray-100 bg-gray-50/50">{["ORDER","CUSTOMER","SHIPPING ADDRESS","AMOUNT","ITEMS","STATUS"].map(col => (<th key={col} className="py-2.5 px-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">{col}</th>))}</tr></thead>
