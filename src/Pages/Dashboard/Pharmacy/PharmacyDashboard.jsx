...
function MainLiveMapPanel({ user, pendingOrder, locationSocketRef }) {
  ...
  useEffect(() => {
    ...
    const onUserLoc = ({ latitude, longitude }) => { setCustomerPos([latitude, longitude]); setCustomerOnline(true); };
    const onOffline = ({ role }) => { if (role === "user") setCustomerOnline(false); };
    currentSocket.on("userLocation", onUserLoc);
    currentSocket.on("participantOffline", onOffline);
    return () => { currentSocket.off("userLocation", onUserLoc); currentSocket.off("participantOffline", onOffline); };
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-7">
        ...
        <div className="py-14 flex flex-col items-center justify-center bg-gray-50/60">
          ...
          <p className="text-[13px] font-bold text-gray-500 text-center px-4">Live delivery map appears when you have a pending order</p>
          <p className="text-[11px] text-gray-400 mt-1 text-center px-4">Your GPS location will be shared with the customer automatically</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-7">
      <div className="px-4 sm:px-5 py-3.5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center gap-2 justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-[15px] font-black text-gray-900">Live Order Tracking</p>
            <p className="text-[11px] text-gray-400">Order #{pendingOrder._id?.slice(-8).toUpperCase()} · {pendingOrder.userId?.name || "Customer"}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {customerOnline && (
            <span className="flex items-center gap-1.5 text-[11px] font-bold text-blue-600 bg-blue-50 border border-blue-100 px-2.5 py-1 rounded-full">
              <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />Customer live
            </span>
          )}
          {pharmacyPos && (
            <span className="flex items-center gap-1.5 text-[11px] font-bold text-green-700 bg-green-50 border border-green-100 px-2.5 py-1 rounded-full">
              <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />You're live
            </span>
          )}
        </div>
      </div>
      {route && (
        <div className="flex items-center gap-4 px-5 py-2 bg-blue-50 border-b border-blue-100">
          ...
          <span className="text-[12px] font-bold text-blue-700">⏱ ~{route.duration} min</span>
          <button onClick={() => setRoute(null)} className="ml-auto text-[11px] text-blue-400 hover:text-blue-600 font-semibold transition">✕ Clear</button>
        </div>
      )}
      <div className="relative">
        <MapContainer center={pharmacyPos || DEFAULT_CENTER} zoom={14} style={{ height: "clamp(200px, 40vw, 320px)", width: "100%" }} zoomControl={true} scrollWheelZoom={true} attributionControl={true}>
          <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
          {visiblePositions.length > 0 && <BoundsFitter positions={visiblePositions} />}
          {pharmacyPos && <Marker position={pharmacyPos} icon={PHARMACY_ICON}><Popup><div className="text-[13px] font-semibold">🏥 {user?.name || "Your Pharmacy"}</div></Popup></Marker>}
          {customerPos && <Marker position={customerPos} icon={USER_ICON}><Popup><div className="text-[13px] font-semibold">📍 {pendingOrder.userId?.name || "Customer"}</div></Popup></Marker>}
          {route && <Polyline positions={route.coords} pathOptions={{ color: "#3b82f6", weight: 5, opacity: 0.85, dashArray: "10 5" }} />}
        </MapContainer>
      </div>
      <div className="px-4 sm:px-5 py-3 bg-gray-950 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
        <p className="text-[11px] text-gray-400">
          {pharmacyPos && customerPos ? "Both locations loaded — get the route" : pharmacyPos ? "Waiting for customer pin…" : "Acquiring GPS…"}
        </p>
        <div className="flex items-center gap-2">
          {route && <span className="text-[11px] text-green-400 font-bold">Route active ✓</span>}
          {pharmacyPos && customerPos && !route && (
            <button onClick={handleGetRoute} disabled={routeLoading} className="flex items-center gap-2 bg-green-500 hover:bg-green-400 disabled:opacity-50 text-white text-[12px] font-bold px-4 py-2 rounded-xl transition">
              {routeLoading ? <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg>}
              {routeLoading ? "Loading…" : "Get Route"}
            </button>
          )}
          {route && (
            <button onClick={handleGetRoute} disabled={routeLoading} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-[12px] font-bold px-4 py-2 rounded-xl transition">
...
function Sidebar({ user, active, onLogout, navigate, pendingOrder, locationSocketRef, sidebarOpen, setSidebarOpen }) {
  const NAV = [
  ...
  return (
    <aside className={`fixed left-0 top-0 bottom-0 z-30 w-[220px] bg-white border-r border-gray-100 flex flex-col flex-shrink-0 overflow-y-auto transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
...
function OrderRow({ order }) {
  ...
  const statusMap = { pending: "bg-amber-100 text-amber-700", delivered: "bg-green-100 text-green-700", cancalled: "bg-red-100 text-red-600", cancelled: "bg-red-100 text-red-600" };
      ...
      <tr className="border-b border-gray-50 hover:bg-gray-50/60 transition-colors cursor-pointer" onClick={() => setExpanded(e => !e)}>
        <td className="py-3 px-4">
          <div className="flex items-center gap-2.5">
            ...
            <div><p className="text-[13px] font-bold text-gray-800">#{order._id?.slice(-8).toUpperCase()}</p><p className="text-[10px] text-gray-400">{new Date(order.createdAt).toLocaleDateString("en-NP", { day: "numeric", month: "short" })}</p></div>
          </div>
        </td>
        <td className="py-3 px-4 hidden sm:table-cell"><p className="text-[13px] text-gray-700 font-medium truncate max-w-[110px]">{order.userId?.name || "Customer"}</p></td>
        <td className="py-3 px-4 hidden md:table-cell"><p className="text-[12px] text-gray-500 truncate max-w-[140px]">{order.shippingAddress || "—"}</p></td>
        <td className="py-3 px-4"><p className="text-[13px] font-bold text-green-600">Rs. {order.totalAmount?.toLocaleString()}</p></td>
        <td className="py-3 px-4 hidden sm:table-cell"><p className="text-[13px] text-gray-600">{order.products?.length || 0} item{(order.products?.length || 0) !== 1 ? "s" : ""}</p></td>
        ...
        <td className="py-3 px-4"><svg className={`w-3.5 h-3.5 text-gray-400 transition-transform ${expanded ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></td>
      </tr>
      {expanded && (
          ...
          <td colSpan={7} className="px-4 py-3">
            <div className="space-y-2">
              {order.products?.map((item, i) => {
                ...
                const stock = p.productTotalStockQuantity ?? 0; const outOfStock = stock === 0; const lowStock = !outOfStock && stock <= 5;
                return (
                  <div key={i} className="flex items-center gap-3 bg-white rounded-xl px-3 py-2.5 border border-gray-100">
                    <div className="w-8 h-8 rounded-lg bg-green-50 flex items-center justify-center text-sm flex-shrink-0 overflow-hidden">{p.productImageUrl ? <img src={p.productImageUrl} alt="" className="w-full h-full object-cover rounded-lg" /> : "💊"}</div>
                    <div className="flex-1 min-w-0"><p className="text-[12px] font-bold text-gray-800 truncate">{p.productName}</p><p className="text-[11px] text-gray-400">Ordered: {item.quantity} × Rs. {p.productPrice?.toLocaleString()}</p></div>
                    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold flex-shrink-0 ${outOfStock ? "bg-red-50 text-red-600 border border-red-100" : lowStock ? "bg-orange-50 text-orange-600 border border-orange-100" : "bg-green-50 text-green-700 border border-green-100"}`}>
                      {outOfStock ? "🚨" : lowStock ? "⚠️" : "📦"}{outOfStock ? "Out of Stock" : `${stock} left`}
                    </div>
                  </div>
                );
              })}
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-md overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-[12px] font-bold text-gray-700 mb-1.5">Price (Rs.) *</label><input type="number" name="productPrice" value={form.productPrice} onChange={handleChange} placeholder="150" min="0" className="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-green-400/40 focus:border-green-400 bg-white transition" /></div>
            <div><label className="block text-[12px] font-bold text-gray-700 mb-1.5">Stock Qty *</label><input type="number" name="productTotalStockQuantity" value={form.productTotalStockQuantity} onChange={handleChange} placeholder="100" min="0" className="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-green-400/40 focus:border-green-400 bg-white transition" /></div>
          </div>
...
export default function PharmacyDashboard() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      setOrders(data); setRecentOrders(data.slice(0, 6));
  ...
  useEffect(() => {
    ...
    const socket = io("https://keshab-sigdel-health-haul-backend-production.up.railway.app", { query: { userId: user._id, role: "pharmacy" }, withCredentials: true });
    locationSocketRef.current = socket;
    socket.emit("joinOrderRoom", pendingOrder._id);
    if (navigator.geolocation) {
      geoWatchRef.current = navigator.geolocation.watchPosition(
        ({ coords: { latitude, longitude } }) => {
          socket.emit("pharmacyShareLocation", { orderId: pendingOrder._id, latitude, longitude, pharmacyName: user.name });
    ...
    return () => {
      if (geoWatchRef.current) { navigator.geolocation.clearWatch(geoWatchRef.current); geoWatchRef.current = null; }
      if (locationSocketRef.current) { locationSocketRef.current.emit("leaveOrderRoom", pendingOrder._id); locationSocketRef.current.disconnect(); locationSocketRef.current = null; }
    };
  }, [orders, user]);
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa]">
      ...
      <Sidebar user={user} active="dashboard" onLogout={logout} navigate={navigate} pendingOrder={pendingOrder} locationSocketRef={locationSocketRef} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} />
      ...
      <div className="lg:pl-[220px] pt-[52px] lg:pt-0">
        <main className="px-4 sm:px-8 py-7 min-h-screen">
          <div className="mb-7 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            <div>
              <h1 className="text-[22px] sm:text-[26px] font-black text-gray-900 tracking-tight leading-tight">Pharmacy Dashboard</h1>
              <p className="text-gray-400 text-[13px] mt-0.5">Real-time overview of your orders and revenue</p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <button onClick={fetchOrders} className="flex items-center gap-2 border border-gray-200 bg-white text-gray-600 px-3 sm:px-4 py-2.5 rounded-xl font-bold text-[13px] hover:border-green-300 hover:text-green-700 transition shadow-sm">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                Refresh
              </button>
              <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 bg-gray-950 text-white px-3 sm:px-4 py-2.5 rounded-xl font-bold text-[13px] hover:bg-gray-800 transition shadow-sm">
          ...
          {loading ? (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
              {[...Array(4)].map((_, i) => <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 animate-pulse"><div className="w-9 h-9 bg-gray-100 rounded-xl mb-3" /><div className="h-7 bg-gray-100 rounded w-1/2 mb-1" /><div className="h-3 bg-gray-100 rounded w-3/4" /></div>)}
            </div>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
              {STAT_CARDS.map((s) => (
                <div key={s.label} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-3">
                    <div className={`w-9 h-9 ${s.bg} rounded-xl flex items-center justify-center text-lg`}>{s.icon}</div>
                    <span className="text-[10px] font-semibold text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full hidden sm:block">{s.trend}</span>
                  </div>
                  <p className={`text-xl sm:text-2xl font-black ${s.color} leading-none mb-1`}>{s.value}</p>
          ...
          <MainLiveMapPanel user={user} pendingOrder={pendingOrder} locationSocketRef={locationSocketRef} />
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-4 sm:px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-gray-400 mt-0.5">{recentOrders.length} most recent</p>
              </div>
              <button onClick={() => navigate("/pharmacy/orders")} className="text-[12px] font-bold text-gray-500 hover:text-gray-900 bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-lg hover:border-gray-300 transition">View All →</button>
            </div>
            {loading ? (
              ...
              <div className="py-16 text-center"><div className="text-3xl mb-2">📋</div><p className="text-[13px] font-semibold text-gray-600">No orders yet</p></div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full min-w-[480px]">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50/50">
                      {["ORDER", "CUSTOMER", "ADDRESS", "AMOUNT", "ITEMS", "STATUS", ""].map(col => (
                        <th key={col} className={`py-2.5 px-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest ${col === "CUSTOMER" ? "hidden sm:table-cell" : col === "ADDRESS" ? "hidden md:table-cell" : col === "ITEMS" ? "hidden sm:table-cell" : ""}`}>{col}</th>
                      ))}
                    </tr>
                  </thead>
