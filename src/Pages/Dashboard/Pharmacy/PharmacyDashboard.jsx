...
function MainLiveMapPanel({ user, pendingOrder, locationSocketRef }) {
  ...
  useEffect(() => {
    ...
    const lat = Number(
      pendingOrder.deliveryAddress?.lat ??
        pendingOrder.deliveryAddress?.latitude ??
        "",
    );
    const lng = Number(
      pendingOrder.deliveryAddress?.lng ??
        pendingOrder.deliveryAddress?.longitude ??
        "",
    );
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-7">
        ...
        <div className="py-14 flex flex-col items-center justify-center bg-gray-50/60">
          ...
          <p className="text-[13px] font-bold text-gray-500 text-center px-4">
            Live delivery map appears when you have a pending order
          </p>
          <p className="text-[11px] text-gray-400 mt-1 text-center px-4">
            Your GPS location will be shared with the customer automatically
          </p>
        </div>
      </div>
    );
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-7">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400">
              Order #{pendingOrder._id?.slice(-8).toUpperCase()} ·{" "}
      ...
      <div className="relative">
        <MapContainer
          ...
          zoom={14}
          style={{ height: "clamp(200px, 40vw, 320px)", width: "100%" }}
          zoomControl={true}
        ...
        <div className="flex items-center gap-2">
          ...
          {pharmacyPos && customerPos && !route && (
            <button
              ...
              className="flex items-center gap-2 bg-green-500 hover:bg-green-400 disabled:opacity-50 text-white text-[12px] font-bold px-4 py-2 rounded-xl transition"
            >
              {routeLoading ? (
          ...
          {route && (
            <button
              onClick={handleGetRoute}
              disabled={routeLoading}
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-[12px] font-bold px-4 py-2 rounded-xl transition"
            >
              <svg
                className="w-3.5 h-3.5"
                fill="none"
    ...
    <>
      ...
      >
        <td className="py-3 px-4">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-green-50 border border-green-100 flex items-center justify-center text-sm flex-shrink-0">
              💊
            </div>
            <div>
        ...
        <td className="py-3 px-4 hidden sm:table-cell">
          <p className="text-[13px] text-gray-700 font-medium truncate max-w-[110px]">
            {order.userId?.name || "Customer"}
          </p>
        </td>
        <td className="py-3 px-4 hidden md:table-cell">
          <p className="text-[12px] text-gray-500 truncate max-w-[140px]">
        ...
        <td className="py-3 px-4">
          <span
            className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold ${cls}`}
          >
            {label}
          </span>
        </td>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-gray-400">
                        Ordered: {item.quantity} × Rs.{" "}
                        {p.productPrice?.toLocaleString()}
                      </p>
                    </div>
                    <div
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold flex-shrink-0 ${outOfStock ? "bg-red-50 text-red-600 border border-red-100" : lowStock ? "bg-orange-50 text-orange-600 border border-orange-100" : "bg-green-50 text-green-700 border border-green-100"}`}
                    >
...
function AddProductModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      !form.productPrice ||
      !form.productTotalStockQuantity
    ) {
      setError(
        "Name, description, price, and stock quantity are all required.",
      );
      return;
    }
    setSaving(true);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                value={form.productTotalStockQuantity}
                onChange={handleChange}
                placeholder="100"
                min="0"
                className="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-green-400/40 focus:border-green-400 bg-white transition"
          ...
          <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-100 rounded-xl px-3.5 py-3">
            ...
            <p className="text-[11px] text-amber-700 font-medium">
              Product will be marked <strong>Pending</strong> until an admin
              approves it.
            </p>
          </div>
          {error && (
            <p className="text-[12px] font-semibold px-3.5 py-2.5 rounded-xl bg-red-50 text-red-600 border border-red-100">
              {error}
            </p>
          )}
...
export default function PharmacyDashboard() {
  ...
  const cancelledOrders = orders.filter((o) =>
    ["cancalled", "cancelled"].includes(o.orderStatus),
  ).length;
          ...
          <div className="mb-7 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            <div>
              ...
              <p className="text-gray-400 text-[13px] mt-0.5">
                Real-time overview of your orders and revenue
              </p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <button
                onClick={fetchOrders}
                className="flex items-center gap-2 border border-gray-200 bg-white text-gray-600 px-3 sm:px-4 py-2.5 rounded-xl font-bold text-[13px] hover:border-green-300 hover:text-green-700 transition shadow-sm"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              <button
                ...
                className="flex items-center gap-2 bg-gray-950 text-white px-3 sm:px-4 py-2.5 rounded-xl font-bold text-[13px] hover:bg-gray-800 transition shadow-sm"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              {STAT_CARDS.map((s) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div
                      className={`w-9 h-9 ${s.bg} rounded-xl flex items-center justify-center text-lg`}
                    >
                      {s.icon}
                    </div>
                    <span className="text-[10px] font-semibold text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full hidden sm:block">
                      {s.trend}
                    </span>
                  </div>
                  <p
                  ...
                  <div className={`mt-2 ${s.sparkColor}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-7">
            <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <div className="mb-5">
                ...
                <p className="text-[11px] text-gray-400 mt-0.5">
                  Orders processed per day over the last 7 days
                </p>
              </div>
                  ...
                  (day, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div
                            className={`w-full rounded-t-lg transition-all ${i === todayIdx ? "bg-gray-900" : "bg-gray-100 hover:bg-green-100"}`}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col">
              <h2 className="text-[15px] font-black text-gray-900 mb-4">
                Order Status
              </h2>
              <div className="space-y-3 flex-1">
                {[
                  ...
                  <div
                    key={s.label}
                    className={`flex items-center justify-between ${s.bg} rounded-xl px-3.5 py-2.5`}
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[13px] font-black ${s.text}`}>
                      {s.count}
                    </span>
                  </div>
                ))}
                  ...
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50/50">
                      {[
                        ...
                        <th
                          key={col}
                          className={`py-2.5 px-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest ${col === "CUSTOMER" ? "hidden sm:table-cell" : col === "ADDRESS" ? "hidden md:table-cell" : col === "ITEMS" ? "hidden sm:table-cell" : ""}`}
                        >
                          {col}
                        </th>
                      ))}
