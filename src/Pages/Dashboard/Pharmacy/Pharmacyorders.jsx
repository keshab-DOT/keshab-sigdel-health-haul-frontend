      ...
      <div className="grid md:grid-cols-2 gap-5">
          ...
          <div className="space-y-2">
            {order.products?.map((item, i) => (
              <div key={i} className="flex items-center gap-3 bg-white rounded-xl p-3 border border-gray-100">
                <div className="w-9 h-9 bg-green-50 rounded-lg flex items-center justify-center text-sm flex-shrink-0 overflow-hidden">{item.productId?.productImageUrl ? <img src={item.productId.productImageUrl} alt="" className="w-full h-full object-cover rounded-lg" /> : "💊"}</div>
                <div className="flex-1 min-w-0"><p className="text-[13px] font-medium text-gray-800 truncate">{item.productId?.productName || "Product"}</p><p className="text-[11px] text-gray-400">Qty: {item.quantity} × Rs. {item.productId?.productPrice?.toLocaleString()}</p></div>
        ...
        <div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 divide-y divide-gray-50">
            {[{ label: "Customer", value: order.userId?.name || order.customerName || "—" }, { label: "Shipping Address", value: order.shippingAddress || "—" }, { label: "Phone", value: order.phoneNumber || "—" }, { label: "Payment", value: payLabel[order.paymentMethod] || order.paymentMethod || "—" }].map(({ label, value }) => (
...
export default function PharmacyOrders() {
  ...
  const handleStatusUpdate = async (orderId, newStatus) => {
    ...
    try { await api.put(`/orders/update/order/${orderId}`, { orderStatus: newStatus }); setOrders(p => p.map(o => o._id === orderId ? { ...o, orderStatus: newStatus } : o)); showToast("Order status updated!"); }
