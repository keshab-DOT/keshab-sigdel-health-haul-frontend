...
function StatusSelect({ orderId, current, onUpdate, updating }) {
  ...
  return (
    <select value={current} onChange={e => onUpdate(orderId, e.target.value)} disabled={updating}
      className={`border border-gray-200 rounded-lg px-2.5 py-1.5 text-[12px] font-semibold focus:outline-none focus:ring-2 focus:ring-green-400/40 bg-gray-50 transition
        ${updating ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:border-green-300"}`}>
...
export default function PharmacyOrders() {
  ...
  const handleStatusUpdate = async (orderId, newStatus) => {
    ...
    try {
      await api.put(`/orders/update/${orderId}`, { orderStatus: newStatus });
    ...
    <div className="min-h-screen bg-[#f7f8fa]">
      ...
      <div className="pl-[200px]">
        <main className="px-8 py-7 min-h-screen">
          ...
          <div className="mb-6">
            <h1 className="text-[26px] font-black text-gray-900 tracking-tight leading-tight">Orders</h1>
            <p className="text-gray-400 text-[13px] mt-0.5">Manage and update customer order statuses</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            {filtered.length === 0 ? (
              <div className="py-20 text-center">
                ...
                <p className="text-[13px] text-gray-400">{filter === "all" ? "No orders placed yet" : `No ${FILTERS.find(f=>f.key===filter)?.label} orders`}</p>
              </div>
            ) : (
              <>
                <div className="border-b border-gray-100 bg-gray-50/50 px-5 py-3 grid grid-cols-[1.2fr_1fr_90px_100px_130px_28px] gap-3 items-center">
                  {["ORDER", "CUSTOMER", "ITEMS", "AMOUNT", "STATUS", ""].map(col => (
                ...
                <div className="divide-y divide-gray-50">
                  {filtered.map(order => {
                    ...
                    return (
                      <div key={order._id}>
                        <div
                          className="px-5 py-4 grid grid-cols-[1.2fr_1fr_90px_100px_130px_28px] gap-3 items-center hover:bg-gray-50/60 transition-colors cursor-pointer"
                          ...
                          <div className="min-w-0">
                            <p className="text-[13px] font-medium text-gray-700 truncate">{customerName}</p>
                            <p className="text-[10px] text-gray-400 truncate">{order.shippingAddress || "—"}</p>
                          </div>
                          ...
                          <div onClick={e => e.stopPropagation()}>
                            <StatusSelect orderId={order._id} current={order.orderStatus} onUpdate={handleStatusUpdate} updating={updating[order._id]}/>
                          </div>
                ...
                <div className="px-5 py-3 border-t border-gray-100 bg-gray-50/40">
                  <p className="text-[12px] text-gray-400">{filtered.length} of {orders.length} orders shown</p>
                </div>
              </>
            )}
          </div>
