        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-0.5">
              {isEdit ? "Update your product details" : "Requires admin approval before going live"}
            </p>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-sm overflow-hidden">
        <div className="p-6 text-center">
          ...
          <p className="text-[13px] text-gray-500 mb-5">
            "<span className="font-semibold text-gray-700">{product.productName}</span>" will be permanently removed.
          </p>
...
export default function PharmacyProducts() {
  ...
  const [deleteProduct, setDeleteProduct] = useState(null); // product object to delete
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const r = await api.get("/products/get/products");
      // Filter only this pharmacy's products
      const stored = JSON.parse(localStorage.getItem("user"));
      const mine = (r.data || []).filter(p =>
        p.userId?._id === stored?._id || p.userId === stored?._id
      );
      setProducts(mine);
    } catch (_) {
      setProducts([]);
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa]">
      ...
      {addModal && (
        <ProductModal onClose={() => setAddModal(false)} onSuccess={handleModalSuccess} />
      )}
      {editProduct && (
        <ProductModal product={editProduct} onClose={() => setEditProduct(null)} onSuccess={handleModalSuccess} />
      )}
      {deleteProduct && (
        <DeleteModal product={deleteProduct} onClose={() => setDeleteProduct(null)} onSuccess={handleModalSuccess} />
      )}
