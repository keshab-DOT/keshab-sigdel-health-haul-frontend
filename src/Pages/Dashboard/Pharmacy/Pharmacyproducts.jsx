...
const notifMetaN = (type) =>
  TYPE_META_N[type] || { icon: "🔔", color: "bg-gray-50 text-gray-600" };
...
function ProductModal({ product, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    fd.append(
      ...
      Number(form.productTotalStockQuantity),
    );
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-0.5">
              {isEdit
                ? "Product will be sent for re-approval"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-100 rounded-xl px-3.5 py-3">
            ...
            <p className="text-[11px] text-amber-700 font-medium">
              {isEdit
                ? "Editing will reset status to Pending until admin re-approves."
                : "Product will be marked Pending until an admin approves it."}
            </p>
          </div>
          {error && (
        ...
        <div className="p-6 text-center">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={deleting}
              className="flex-1 bg-red-500 text-white py-2.5 rounded-xl font-bold text-[13px] hover:bg-red-600 disabled:opacity-50 transition flex items-center justify-center gap-2"
            >
              {deleting ? (
                <>
                  <svg
...
export default function PharmacyProducts() {
  ...
  const filtered = products.filter(
    (p) => filter === "all" || p.approvalStatus === filter,
  );
  const counts = {
    all: products.length,
        ...
        <main className="px-4 sm:px-8 py-7 min-h-screen max-w-5xl">
          <div className="mb-7 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-gray-950 text-white px-4 py-2.5 rounded-xl font-bold text-[13px] hover:bg-gray-800 transition shadow-sm self-start"
            >
              <svg
                className="w-4 h-4"
                fill="none"
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-0 bg-white rounded-2xl border border-gray-100 shadow-sm mb-5 overflow-hidden">
            {[
              ...
              <div
                key={s.label}
                className={`${s.bg} px-4 sm:px-6 py-4 ${i < 3 ? "border-r border-gray-100" : ""}`}
              >
          ...
          {counts.Rejected > 0 && (
            <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-2xl px-5 py-3.5 mb-4">
              ...
              <p className="text-[13px] font-semibold text-red-800">
                <span className="font-black">{counts.Rejected}</span> product
                {counts.Rejected > 1 ? "s were" : " was"} rejected.
              </p>
              <button
                onClick={() => setFilter("Rejected")}
                className="ml-auto text-[11px] font-bold text-red-700 underline underline-offset-2"
              >
                View
              </button>
            </div>
          ...
          {counts.Pending > 0 && (
            <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-3.5 mb-4">
              ...
              <p className="text-[13px] font-semibold text-amber-800">
                ...
                {counts.Pending > 1 ? "s are" : " is"} awaiting admin approval.
              </p>
              <button
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-20 text-center">
              ...
              {filter === "all" && (
                <button
                  onClick={() => setAddModal(true)}
                  className="inline-flex items-center gap-2 bg-gray-950 text-white px-4 py-2.5 rounded-xl font-bold text-[13px] hover:bg-gray-800 transition"
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
              ...
              {filtered.map((product) => {
                ...
                return (
                  ...
                  >
                    <div className="relative w-full h-36 bg-gradient-to-br from-green-50 to-emerald-50 flex items-center justify-center overflow-hidden">
                      ...
                      {product.approvalStatus === "Rejected" && (
                        <div className="absolute inset-0 bg-red-900/10 flex items-end pb-3 justify-center">
                          <span className="text-[10px] font-bold text-red-700 bg-white/90 px-2.5 py-1 rounded-full">
                            Edit to resubmit
                          </span>
                        </div>
                      )}
                    ...
                    <div className="p-4">
                      <h3 className="text-[14px] font-black text-gray-900 truncate mb-1">
                        {product.productName}
                      </h3>
                      ...
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <p className="text-[11px] text-gray-500 font-medium bg-gray-50 px-2.5 py-1 rounded-full border border-gray-100">
                          Stock:{" "}
                          <span
                            className={`font-bold ${product.productTotalStockQuantity === 0 ? "text-red-500" : "text-gray-700"}`}
                          >
