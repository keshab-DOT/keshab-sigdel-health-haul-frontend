...
function NotificationBell({ userId }) {
  ...
  useEffect(() => { if (!userId) return; const socket = io("http://localhost:3000", { query: { userId }, withCredentials: true }); socket.emit("joinUserRoom", userId); socket.on("newNotification", (n) => { setNotifs(prev => prev.some(x => x._id === n._id) ? prev : [n, ...prev]); setUnread(prev => prev + 1); }); return () => { socket.emit("leaveUserRoom", userId); socket.disconnect(); }; }, [userId]);
...
const PAYMENT_METHODS = [
  { val: "cod", label: "Cash on Delivery", icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg> },
  { val: "khalti", label: "Khalti", icon: <span className="text-[15px]">💳</span> },
];
...
export default function CartPage() {
  ...
  const updateQty = async (id, qty) => {
    ...
    try { await api.put(`/cart/update/cartitem/${id}`, { quantity: qty }); setCartItems(p => p.map(item => item._id === id ? { ...item, quantity: qty } : item)); }
  ...
  const validatePhone = (phone) => { const digits = phone.replace(/\D/g, ""); if (digits.length !== 10) return "Phone number must be exactly 10 digits"; return ""; };
  const handlePhoneChange = (e) => { const val = e.target.value; if (/^\d{0,10}$/.test(val)) { setCheckoutForm(p => ({ ...p, phoneNumber: val })); setPhoneError(val.length > 0 && val.length < 10 ? "Phone number must be 10 digits" : ""); } };

  const handleCheckout = async (e) => {
    e.preventDefault();
    if (!checkoutForm.shippingAddress.trim()) { showToast("Please enter your shipping address", "error"); return; }
    ...
    try {
      ...
      setCartItems([]); setShowCheckout(false); showToast("Order placed successfully! 🎉");
      setTimeout(() => navigate("/user/orders"), 1500);
    } catch (err) { showToast(err.response?.data?.message || err.message || "Checkout failed", "error"); }
  ...
  if (loading) return (
    <div className="flex min-h-screen items-center justify-center bg-gray-50">
      <div className="text-center"><div className="w-10 h-10 border-[3px] border-green-500 border-t-transparent rounded-full animate-spin mx-auto mb-3" /><p className="text-gray-400 text-sm">Loading your cart…</p></div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="fixed inset-0 bg-black/40 z-40 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
              <div><h3 className="text-[15px] font-black text-gray-900">Complete Your Order</h3><p className="text-[11px] text-gray-400 mt-0.5">Fill in your delivery details</p></div>
              <button onClick={() => setShowCheckout(false)} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            <form onSubmit={handleCheckout} className="p-6 space-y-4">
              <div>
                <label className="block text-[13px] font-bold text-gray-700 mb-1.5">Shipping Address</label>
                <textarea rows={3} placeholder="Enter your full delivery address" value={checkoutForm.shippingAddress} onChange={e => setCheckoutForm(p => ({ ...p, shippingAddress: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-green-400/40 focus:border-green-400 resize-none bg-gray-50/50 transition" required />
              </div>
              <div>
                ...
                <input type="tel" placeholder="98XXXXXXXX" value={checkoutForm.phoneNumber} onChange={handlePhoneChange} maxLength={10} className={`w-full border rounded-xl px-3.5 py-2.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-green-400/40 bg-gray-50/50 transition ${phoneError ? "border-red-300 focus:border-red-400" : "border-gray-200 focus:border-green-400"}`} required />
                {phoneError && <p className="text-[11px] text-red-500 mt-1 font-medium">{phoneError}</p>}
                {!phoneError && checkoutForm.phoneNumber.length === 10 && <p className="text-[11px] text-green-600 mt-1 font-medium">✓ Valid phone number</p>}
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {PAYMENT_METHODS.map(({ val, label, icon }) => (
                    <button key={val} type="button" onClick={() => setCheckoutForm(p => ({ ...p, paymentMethod: val }))}
                      className={`flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-2 text-[11px] font-bold transition-all ${checkoutForm.paymentMethod === val ? val === "khalti" ? "border-purple-500 bg-purple-50 text-purple-700" : "border-gray-800 bg-gray-900 text-white" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>
                      {icon}{label}
                    </button>
                  ))}
                </div>
                {checkoutForm.paymentMethod === "khalti" && (
                  <div className="mt-2 flex items-start gap-2 bg-purple-50 border border-purple-100 rounded-xl px-3.5 py-2.5">
                    ...
                    <p className="text-[11px] text-purple-700 leading-relaxed">You'll be redirected to Khalti. <strong>Your order will be cancelled if payment is not completed.</strong></p>
                  </div>
                )}
              </div>
              ...
              <button type="submit" disabled={checkoutLoading || checkoutForm.phoneNumber.length !== 10}
                ...
                {checkoutLoading ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Processing…</> : checkoutForm.paymentMethod === "khalti" ? "Pay with Khalti 💜" : "Place Order →"}
              </button>
            </form>
          </div>
        </div>
      ...
      <main className="flex-1 px-8 py-6 space-y-5">
        <div className="flex items-center justify-between">
          <div><h2 className="text-[22px] font-black text-gray-900 tracking-tight">My Cart</h2><p className="text-gray-400 text-[13px] mt-0.5">{cartItems.length} item{cartItems.length !== 1 ? "s" : ""} in your cart</p></div>
          <div className="flex gap-2">
            <button onClick={() => navigate("/user/search")} className="flex items-center gap-2 border border-gray-200 text-gray-600 px-3.5 py-2 rounded-xl hover:border-green-300 hover:text-green-700 hover:bg-green-50 transition text-[13px] font-medium"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>Add More</button>
            {cartItems.length > 0 && <button onClick={clearCart} className="flex items-center gap-1.5 border border-red-200 text-red-500 px-3.5 py-2 rounded-xl hover:bg-red-50 transition text-[13px] font-medium"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>Clear</button>}
          </div>
        </div>
                ...
                return (
                  <div key={item._id} className={`bg-white rounded-xl border shadow-sm p-4 flex items-center gap-4 transition-shadow ${outOfStock ? "border-red-100 opacity-70" : "border-gray-100 hover:shadow-md"}`}>
                    <div className="w-14 h-14 rounded-xl bg-green-50 flex items-center justify-center flex-shrink-0 overflow-hidden border border-green-100">
                      {src ? <img src={src} alt={p?.productName} className="w-full h-full object-cover rounded-xl" onError={e => { e.target.style.display = "none"; }} /> : <span className="text-2xl">💊</span>}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {outOfStock ? <p className="text-[11px] text-red-500 font-semibold mt-0.5">Out of stock — please remove</p> : <p className="text-[11px] text-gray-400 mt-0.5">{p?.productTotalStockQuantity} in stock</p>}
                    </div>
            ...
            <div className="w-72 flex-shrink-0 sticky top-20">
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                ...
                {cartItems.some(i => i.productId?.productTotalStockQuantity === 0) ? (
                  <div className="bg-red-50 border border-red-100 rounded-xl px-3.5 py-3 mb-2 text-center"><p className="text-[12px] text-red-600 font-semibold">Remove out-of-stock items to checkout</p></div>
                ) : (
