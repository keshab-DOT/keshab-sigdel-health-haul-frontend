...
/* ─── Mobile Nav Drawer ─────────────────────────────────────────── */
function MobileNav({ open, onClose, navigate, user, onLogout }) {
  const links = [
    { label: "Dashboard", path: "/user/dashboard" },
    { label: "Browse Medicines", path: "/user/search" },
        ...
        <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
          {links.map(({ label, path }) => (
            <button
              ...
              className="w-full text-left px-4 py-3 text-[13px] font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-xl transition"
            >
              {label}
            </button>
          ))}
...
function Topbar({ user, cartCount, onLogout, navigate }) {
  ...
  return (
    <>
      <MobileNav open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} navigate={navigate} user={user} onLogout={onLogout} />
...
export default function CartPage() {
  ...
  // Mobile: show/hide order summary
  ...
  const hasOutOfStock = cartItems.some((i) => i.productId?.productTotalStockQuantity === 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 left-4 sm:left-auto sm:w-auto z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-lg text-white text-[13px] font-medium ${toast.type === "error" ? "bg-red-500" : "bg-green-600"}`}>
          {toast.msg}
        </div>
      )}
      ...
      {/* Checkout Modal */}
      {showCheckout && (
        <div className="fixed inset-0 bg-black/40 z-40 flex items-end sm:items-center justify-center sm:p-4 backdrop-blur-sm">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md max-h-[92vh] overflow-y-auto">
            <div className="px-5 sm:px-6 py-4 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
              <div>
            ...
            <form onSubmit={handleCheckout} className="p-5 sm:p-6 space-y-4">
              ...
              <div>
                <label className="block text-[13px] font-bold text-gray-700 mb-1.5">
                  Phone Number <span className="text-[11px] text-gray-400 font-normal">(10 digits)</span>
                </label>
                <input
                  type="tel"
              ...
              <button
                type="submit"
                disabled={checkoutLoading || checkoutForm.phoneNumber.length !== 10}
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-5 sm:py-6 space-y-5">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="text-[20px] sm:text-[22px] font-black text-gray-900 tracking-tight">My Cart</h2>
            <p className="text-gray-400 text-[13px] mt-0.5">{cartItems.length} item{cartItems.length !== 1 ? "s" : ""} in your cart</p>
          </div>
          <div className="flex gap-2 flex-shrink-0">
            <button
              onClick={() => navigate("/user/search")}
              className="flex items-center gap-1.5 sm:gap-2 border border-gray-200 text-gray-600 px-3 sm:px-3.5 py-2 rounded-xl hover:border-green-300 hover:text-green-700 hover:bg-green-50 transition text-[12px] sm:text-[13px] font-medium"
            >
            ...
            {cartItems.length > 0 && (
              <button
                onClick={clearCart}
                className="flex items-center gap-1.5 border border-red-200 text-red-500 px-3 sm:px-3.5 py-2 rounded-xl hover:bg-red-50 transition text-[12px] sm:text-[13px] font-medium"
              >
        ...
        {/* Empty state */}
        {cartItems.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-20 sm:py-24 text-center px-4">
          ...
          <>
            ...
            <div className="flex flex-col lg:flex-row gap-5 items-start">
              ...
              <div className="flex-1 space-y-2.5 w-full">
                {cartItems.map((item) => {
                  ...
                  return (
                    <div
                      key={item._id}
                      className={`bg-white rounded-xl border shadow-sm p-3.5 sm:p-4 flex items-center gap-3 sm:gap-4 transition-shadow ${outOfStock ? "border-red-100 opacity-70" : "border-gray-100 hover:shadow-md"}`}
                    >
                      {/* Product image */}
                      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-green-50 flex items-center justify-center flex-shrink-0 overflow-hidden border border-green-100">
                        {src
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {outOfStock
                          ...
                          : <p className="text-[11px] text-gray-400 mt-0.5 hidden sm:block">{p?.productTotalStockQuantity} in stock</p>}
                      </div>
                      ...
                      <div className={`flex items-center border rounded-xl overflow-hidden ${outOfStock ? "border-gray-100 opacity-40 pointer-events-none" : "border-gray-200"}`}>
                        <button
                          ...
                          disabled={item.quantity <= 1 || updating[item._id] || outOfStock}
                          className="w-8 h-8 flex items-center justify-center text-gray-500 hover:bg-gray-50 disabled:opacity-30 font-bold text-base transition"
                        >−</button>
                        <span className="w-8 text-center font-black text-gray-800 text-[13px]">{item.quantity}</span>
                        <button
                          onClick={() => updateQty(item._id, item.quantity + 1)}
                          disabled={item.quantity >= p?.productTotalStockQuantity || updating[item._id] || outOfStock}
                          className="w-8 h-8 flex items-center justify-center text-gray-500 hover:bg-gray-50 disabled:opacity-30 font-bold text-base transition"
                        >+</button>
                      </div>

                      {/* Line total - hidden on smallest screens */}
                      <p className="font-black text-gray-800 w-20 sm:w-24 text-right text-[12px] sm:text-[13px] hidden xs:block flex-shrink-0">
                        Rs. {((p?.productPrice || 0) * item.quantity).toLocaleString()}
                      </p>
              ...
              <div className="hidden lg:block w-72 flex-shrink-0 sticky top-20">
                <OrderSummaryCard
                  ...
                  onContinue={() => navigate("/user/search")}
                />
              </div>
            </div>

            {/* Mobile: collapsible order summary + sticky CTA */}
            <div className="lg:hidden">
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-[13px] font-black text-gray-900">Order Summary</span>
                    <span className="text-[11px] text-gray-400 font-medium">{cartItems.length} item{cartItems.length !== 1 ? "s" : ""}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-[15px] font-black text-green-600">Rs. {total.toLocaleString()}</span>
                  ...
                  <div className="px-4 pb-4 border-t border-gray-50 space-y-2 pt-3">
                    {cartItems.map((item) => (
                      <div key={item._id} className="flex justify-between text-[12px]">
                        <span className="text-gray-500 truncate max-w-[60%]">{item.productId?.productName} × {item.quantity}</span>
              ...
              <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 py-3 z-20 flex gap-3 shadow-lg">
                <button onClick={() => navigate("/user/search")} className="flex-1 border border-gray-200 text-gray-500 py-2.5 rounded-xl text-[13px] font-medium hover:bg-gray-50 transition">
                  Continue Shopping
                </button>
                {hasOutOfStock ? (
                  <div className="flex-1 bg-red-50 border border-red-100 rounded-xl px-3 py-2.5 text-center">
                    <p className="text-[11px] text-red-600 font-semibold leading-tight">Remove out-of-stock items first</p>
                  </div>
                ) : (
                  <button onClick={() => setShowCheckout(true)} className="flex-1 bg-gray-900 text-white py-2.5 rounded-xl font-black hover:bg-gray-800 transition text-[13px]">
                    Checkout →
                  </button>
                )}
              </div>
...
function OrderSummaryCard({ cartItems, total, hasOutOfStock, onCheckout, onContinue }) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
      ...
      {hasOutOfStock ? (
        ...
        <button onClick={onCheckout} className="w-full bg-gray-900 text-white py-2.5 rounded-xl font-black hover:bg-gray-800 transition mb-2 text-[13px]">
          Checkout →
        </button>
      )}
