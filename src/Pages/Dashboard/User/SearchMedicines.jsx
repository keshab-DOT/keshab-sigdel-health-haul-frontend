        ...
        <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
          {links.map(({ label, path }) => (
            <button
              ...
              className={`w-full text-left px-4 py-3 text-[13px] font-medium rounded-xl transition ${path === "/user/search" ? "bg-gray-100 text-gray-900 font-semibold" : "text-gray-600 hover:text-gray-900 hover:bg-gray-50"}`}
            >
              {label}
            </button>
          ))}
...
function ReviewModal({ pharmacy, onClose, onSubmitted, currentUserId }) {
  ...
  const fetchReviews = async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/reviews/pharmacy/${pharmacy._id}`);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center sm:p-4 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        <div className="px-5 sm:px-6 py-4 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="text-[12px] font-bold text-gray-700">{avgRating > 0 ? avgRating.toFixed(1) : "—"}</span>
              <span className="text-[11px] text-gray-400">({totalCount} review{totalCount !== 1 ? "s" : ""})</span>
            </div>
          </div>
          ...
          <div className="px-5 sm:px-6 py-4 border-b border-gray-100 bg-gray-50/50">
            {alreadyReviewed ? (
              <div className="flex items-center gap-2 text-green-700 bg-green-50 border border-green-100 rounded-xl px-3.5 py-2.5">
                ...
                <span className="text-[12px] font-semibold">You've already reviewed this pharmacy.</span>
              </div>
            ) : (
...
function ProductCard({ product, onAddToCart, adding, onReviewClick, pharmacyRatings }) {
  ...
  const src = product.productImageUrl
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-all duration-200 flex flex-col group">
      <div className="h-32 sm:h-36 bg-gradient-to-br from-green-50 to-emerald-50 flex items-center justify-center overflow-hidden relative">
        ...
        {lowStock && (
          <div className="absolute top-2 right-2 bg-orange-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-md">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden flex-shrink-0">
            <button
              ...
              className="w-7 h-7 flex items-center justify-center text-gray-500 hover:bg-gray-50 disabled:opacity-30 transition font-bold"
            >−</button>
            <span className="w-6 text-center text-[12px] font-bold text-gray-800">{qty}</span>
            <button
              onClick={() => setQty((q) => Math.min(product.productTotalStockQuantity, q + 1))}
...
export default function SearchMedicines() {
  ...
  const fetchPharmacyRatings = async (prods) => {
    ...
    ids.forEach((id, i) => {
      if (results[i].status === "fulfilled") {
        const { averageRating, totalCount } = results[i].value.data;
        if (totalCount > 0) map[id] = { averageRating, totalCount };
      }
    });
  ...
  const handleReviewSubmitted = (pharmacyId, newReview) => {
    setPharmacyRatings((prev) => {
      ...
      const newAvg = Math.round(((existing.averageRating * existing.totalCount + newReview.rating) / newCount) * 10) / 10;
      return { ...prev, [pharmacyId]: { averageRating: newAvg, totalCount: newCount } };
    });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-5 sm:py-6 space-y-4">
        {/* Page header */}
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="text-[20px] sm:text-[22px] font-black text-gray-900 tracking-tight">Browse Medicines</h2>
          ...
          <button
            ...
            className="relative flex items-center gap-2 bg-gray-900 text-white px-3.5 sm:px-4 py-2.5 rounded-xl hover:bg-gray-800 transition shadow-sm text-[13px] font-bold flex-shrink-0"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-3.5 space-y-2.5">
          ...
          <div className={`sm:flex gap-2.5 ${filtersOpen ? "flex flex-col sm:flex-row" : "hidden sm:flex"}`}>
            <select
              ...
              className="flex-1 sm:flex-none border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-green-400/40 bg-gray-50/50 text-gray-600 sm:min-w-[120px]"
            >
            ...
            <select
              ...
              className="flex-1 sm:flex-none border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-green-400/40 bg-gray-50/50 text-gray-600 sm:min-w-[145px]"
            >
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm py-20 sm:py-24 text-center px-4">
            ...
            {query && (
              <button onClick={() => setQuery("")} className="bg-gray-900 text-white px-5 py-2 rounded-xl font-bold text-[13px] hover:bg-gray-800 transition">
                Clear Search
              </button>
            )}
          </div>
