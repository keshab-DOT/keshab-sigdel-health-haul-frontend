...
export default function SearchMedicines() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      api.get("/categories/"), // correct URL matches categoryRoutes mounted at /api/categories
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      r = r.filter(p => p.productName?.toLowerCase().includes(q) || p.productDescription?.toLowerCase().includes(q));
    }
