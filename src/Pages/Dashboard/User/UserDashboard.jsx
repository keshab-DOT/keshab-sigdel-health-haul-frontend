...
// Safely extract an array no matter what shape the API returns
...
function StatusPill({ status }) {
  const map = {
    ...
    ontheway: { cls: "bg-blue-100 text-blue-700", dot: "bg-blue-400", label: "On The Way" },
    ...
    <div className="relative rounded-2xl overflow-hidden h-full min-h-[260px] bg-gradient-to-br from-gray-900 via-gray-800 to-emerald-950 flex flex-col justify-between p-5">
      ...
      <div className="relative flex items-start justify-between gap-3">
        ...
        <div className="w-14 h-14 rounded-xl overflow-hidden border border-white/20 bg-white/10 flex items-center justify-center flex-shrink-0">
          {productImage
            ? <img src={`http://localhost:3000/${productImage}`} alt={productName || "Medicine"} className="w-full h-full object-cover" onError={e => { e.target.style.display = "none"; }} />
            ...
            <div className="flex items-center mb-2">
              {steps.map((step, i) => (
                <div key={step} className="flex items-center flex-1 last:flex-none">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[9px] font-black border-2 transition-all ${i < idx ? "bg-green-400 border-green-400 text-white" : i === idx ? "bg-white border-white text-gray-900" : "border-white/20 bg-white/5 text-white/30"}`}>
                    {i < idx ? "✓" : i + 1}
                  </div>
...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      api.get("/categories/"),
    ]).then(([ordRes, cartRes, prodRes, catRes]) => {

      if (ordRes.status === "fulfilled") {
        const raw = extractArray(ordRes.value.data);
        setOrders(raw.map(o => ({ ...o, orderStatus: o.orderStatus?.toLowerCase() || "pending" })));
      }

      if (cartRes.status === "fulfilled") {
        const cartData = cartRes.value.data;
        setCartCount(Array.isArray(cartData) ? cartData.length : cartData?.length || 0);
      }

      if (prodRes.status === "fulfilled") {
        const prods = extractArray(prodRes.value.data);
        setProducts(prods.slice(0, 4));
      }

      if (catRes.status === "fulfilled") {
        setCategories(extractArray(catRes.value.data));
      }
  ...
  const activeOrders = orders.filter(o => o.orderStatus === "pending" || o.orderStatus === "ontheway");
  ...
  const catIconMap = { "Pain Relief": "💊", "Antibiotics": "🧬", "Vitamins": "🌿", "Heart Care": "❤️", "Skin Care": "✨" };
  const displayCats = categories.length > 0
    ? categories.slice(0, 6)
    : [
      { _id: "1", categoryName: "Pain Relief" },
      { _id: "2", categoryName: "Antibiotics" },
      { _id: "3", categoryName: "Vitamins" },
      { _id: "4", categoryName: "Heart Care" },
      { _id: "5", categoryName: "Skin Care" },
      { _id: "6", categoryName: "All Medicines" },
    ];
      ...
      <main className="flex-1">
        ...
        <section className="px-8 pt-7 pb-6">
          <div className="flex gap-5 items-stretch min-h-[270px]">
            ...
            <div className="flex-1 bg-white rounded-2xl border border-gray-100 shadow-sm px-8 py-7 flex flex-col justify-between relative overflow-hidden">
              <div className="absolute -top-12 -right-12 w-40 h-40 rounded-full bg-green-50 border border-green-100 pointer-events-none" />
              <div className="absolute -bottom-8 -right-4 w-24 h-24 rounded-full bg-emerald-50 pointer-events-none" />
              <div className="relative">
                ...
                <p className="text-gray-500 text-[13px] leading-relaxed max-w-xs">
                  Welcome back, <span className="font-bold text-gray-800">{user?.name?.split(" ")[0]}</span>. Browse medicines from licensed pharmacies.
                </p>
              </div>
              ...
              return (
                <div key={product._id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all group">
                  ...
                  <div className="p-3">
                    {/* ✅ Category tag */}
                    {catName && (
                      <span className="inline-block mb-1.5 px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 text-[10px] font-bold border border-blue-100">{catName}</span>
                    )}
        ...
        <section className="px-8 pb-6">
          ...
          <div className="grid grid-cols-6 gap-2.5">
            {displayCats.map((cat) => {
              const icon = catIconMap[cat.categoryName] || "🏥";
              const imgSrc = cat.categoryImageUrl
                ? cat.categoryImageUrl.startsWith("http")
                  ? cat.categoryImageUrl
                  : `http://localhost:3000/${cat.categoryImageUrl}`
                : null;
              return (
                <button key={cat._id} onClick={() => navigate("/user/search")}
                  ...
                  <div className="w-10 h-10 rounded-xl bg-green-50 group-hover:bg-green-100 flex items-center justify-center text-xl transition overflow-hidden">
                    {imgSrc
                      ? <img src={imgSrc} alt={cat.categoryName} className="w-full h-full object-cover rounded-xl" />
                      : icon}
                  </div>
                  <p className="text-[11px] font-semibold text-gray-700 text-center leading-tight">{cat.categoryName}</p>
                </button>
              );
            })}
          </div>
              ...
              {orders.slice(0, 5).map(order => (
                <div key={order._id} onClick={() => navigate("/user/orders")} className="flex items-center gap-3 px-4 py-3.5 hover:bg-gray-50 transition cursor-pointer group">
                  <div className="w-8 h-8 bg-gray-50 group-hover:bg-green-50 rounded-lg flex items-center justify-center text-sm flex-shrink-0 transition">
                    {order.orderStatus === "delivered" ? "✅" : order.orderStatus === "cancalled" ? "❌" : order.orderStatus === "ontheway" ? "🚚" : "📦"}
                  </div>
