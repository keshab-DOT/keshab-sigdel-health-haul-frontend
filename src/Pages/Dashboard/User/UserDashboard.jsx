  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (ordRes.status === "fulfilled") {
        const ordData = extractArray(ordRes.value.data);
        setOrders(ordData.map(o => ({ ...o, orderStatus: o.orderStatus?.toLowerCase() || "pending" })));
      }

      // ── Cart ────────────────────────────────────────────────────────────────
      if (cartRes.status === "fulfilled") {
        const cartData = cartRes.value.data;
        // cartData could be an array of items, or an object with a length/count field
        if (Array.isArray(cartData)) {
          setCartCount(cartData.length);
        } else {
          setCartCount(cartData?.length ?? cartData?.count ?? cartData?.total ?? 0);
        }
      }

      // ── Products ────────────────────────────────────────────────────────────
      if (prodRes.status === "fulfilled") {
        const prodArray = extractArray(prodRes.value.data);
        setProducts(prodArray.slice(0, 4));
      }
