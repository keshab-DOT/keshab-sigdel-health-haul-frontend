...
export default function AdminUsers() {
  ...
  const getRole = (u) =>
    (Array.isArray(u.roles) ? u.roles[0] : u.roles || "user").toLowerCase();
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa]">
      {toast && (
        <div
          className={`fixed top-5 right-5 z-50 px-4 py-3 rounded-xl shadow-lg text-white text-[13px] font-bold flex items-center gap-2
        ...
        <main className="px-4 sm:px-6 lg:px-8 py-5 sm:py-7 min-h-screen">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-0 bg-white rounded-2xl border border-gray-100 shadow-sm mb-5 overflow-hidden">
            {[
              ...
              <div
                key={s.label}
                className={`${s.bg} px-4 sm:px-6 py-4 ${i % 2 === 0 ? "border-r border-gray-100" : ""} ${i >= 2 ? "border-t sm:border-t-0 border-gray-100" : ""} sm:border-r sm:last:border-r-0`}
              >
              ...
              {FILTERS.map(({ key, label }) => {
                ...
                return (
                  <button
                    ...
                    className={`flex-shrink-0 px-3 py-2 rounded-xl text-[12px] sm:text-[13px] font-medium transition-all flex items-center gap-1.5
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="hidden sm:grid border-b border-gray-100 bg-gray-50/50 px-5 py-3 grid-cols-[1.5fr_1fr_1fr_110px_160px] gap-3 rounded-t-2xl">
              {["USER", "ROLE", "JOINED", "STATUS", "CHANGE STATUS"].map(
                (col) => (
                  <p
                    key={col}
                      ...
                      <div className="hidden sm:grid px-5 py-4 grid-cols-[1.5fr_1fr_1fr_110px_160px] gap-3 items-center hover:bg-gray-50/40 transition-colors">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p
                              className={`text-[13px] font-bold truncate ${status !== "Active" ? "text-gray-400" : "text-gray-800"}`}
                            >
                              {user.name}
                            </p>
                        ...
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold w-fit capitalize border ${roleCls}`}
                        >
                          {role}
                        </span>
                        ...
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold w-fit border ${statusCls}`}
                        >
                          <span
                          ...
                          {isAdmin ? (
                            ...
                            <div className="flex items-center gap-2">
                              <select
                                ...
                                defaultValue={status}
                                onChange={(e) =>
                                  changeStatus(user._id, e.target.value)
                                }
                              ...
                              {acting[user._id] && (
                                <svg
                                  className="animate-spin w-4 h-4 text-green-500 flex-shrink-0"
                                  fill="none"
                                  viewBox="0 0 24 24"
                                >
                                  <circle
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap mb-1">
                              ...
                              <span
                                className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold capitalize border ${roleCls}`}
                              >
                                {role}
                              </span>
                              <span
                                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${statusCls}`}
                              >
                                <span
            ...
            {!loading && filtered.length > 0 && (
              <div className="px-4 sm:px-5 py-3 border-t border-gray-100 bg-gray-50/40 rounded-b-2xl">
