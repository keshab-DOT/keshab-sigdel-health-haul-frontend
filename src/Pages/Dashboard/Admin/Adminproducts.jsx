...
function ProductDetail({ product, onApprove, onReject, acting }) {
  ...
  const imgSrc = product.productImageUrl
    ? product.productImageUrl.startsWith("http") ? product.productImageUrl
      : `https://keshab-sigdel-health-haul-backend-production.up.railway.app/uploads/${product.productImageUrl}`
    : null;
  return (
    <div className="border-t border-gray-100 bg-gray-50/50 px-4 sm:px-6 py-4 sm:py-5">
      <div className="flex flex-col lg:grid lg:grid-cols-3 gap-5 sm:gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex gap-4">
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-white border border-gray-100 flex items-center justify-center text-4xl overflow-hidden flex-shrink-0 shadow-sm">
              {imgSrc ? <img src={imgSrc} alt="" className="w-full h-full object-cover rounded-2xl" /> : "💊"}
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-[15px] sm:text-[16px] font-black text-gray-900 mb-1">{product.productName}</h3>
              <p className="text-[12px] sm:text-[13px] text-gray-500 leading-relaxed mb-3">{product.productDescription || "No description provided."}</p>
              <div className="flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-1.5 bg-green-50 border border-green-100 text-green-700 text-[11px] sm:text-[12px] font-bold px-2.5 sm:px-3 py-1 rounded-full">💰 Rs. {product.productPrice?.toLocaleString()}</span>
                <span className={`inline-flex items-center gap-1.5 text-[11px] sm:text-[12px] font-bold px-2.5 sm:px-3 py-1 rounded-full border ${product.productTotalStockQuantity > 0 ? "bg-blue-50 border-blue-100 text-blue-700" : "bg-red-50 border-red-100 text-red-600"}`}>📦 Stock: {product.productTotalStockQuantity}</span>
                {product.categoryId?.categoryName && (
                  <span className="inline-flex items-center gap-1.5 bg-blue-50 border border-blue-100 text-blue-700 text-[11px] sm:text-[12px] font-bold px-2.5 sm:px-3 py-1 rounded-full">🏷️ {product.categoryId.categoryName}</span>
                )}
              </div>
            </div>
          </div>
          {product.approvalStatus === "Pending" && (
            <div className="flex flex-col sm:flex-row gap-3 pt-1">
              <button onClick={() => onApprove(product._id)} disabled={acting}
                ...
                {acting === product._id + "Approved" ? <><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Approving…</> : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Approve — Allow Selling</>}
              </button>
              <button onClick={() => onReject(product._id)} disabled={acting}
                ...
                {acting === product._id + "Rejected" ? <><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Rejecting…</> : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>Reject — Block Selling</>}
              </button>
            </div>
          )}
          {product.approvalStatus === "Approved" && (
            <button onClick={() => onReject(product._id)} disabled={acting}
              ...
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg>Revoke Approval
            </button>
          )}
          {product.approvalStatus === "Rejected" && (
            <button onClick={() => onApprove(product._id)} disabled={acting}
              ...
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Re-Approve Product
            </button>
          )}
        </div>
        <div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
            ...
            <div className="border-t border-gray-50 pt-3 space-y-2.5">
              <div><p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">Email</p><p className="text-[12px] text-gray-700 font-medium truncate">{pharmacy?.email || "—"}</p></div>
            ...
            <div className={`mt-1 flex items-center gap-2 px-3 py-2.5 rounded-xl text-[12px] font-bold ${product.approvalStatus === "Approved" ? "bg-green-50 text-green-700 border border-green-100" : "bg-red-50 text-red-600 border border-red-100"}`}>
              {product.approvalStatus === "Approved"
                ? <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Permitted to sell</>
                : <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg>Not permitted yet</>}
            </div>
          </div>
        </div>
      </div>
...
export default function AdminProducts() {
  ...
  const handleApproval = async (productId, approvalStatus) => {
    ...
    try {
      ...
      showToast(approvalStatus === "Approved" ? "Product approved — pharmacy can now sell this item." : "Product rejected.", approvalStatus === "Approved" ? "success" : "warn");
      setExpanded(null);
  ...
  const filtered = products.filter((p) => {
    ...
    const matchSearch = !q || p.productName?.toLowerCase().includes(q) || p.userId?.name?.toLowerCase().includes(q) || p.userId?.email?.toLowerCase().includes(q);
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa]">
      {toast && (
        <div className={`fixed top-5 right-5 z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-lg text-white text-[13px] font-bold ${toast.type === "error" ? "bg-red-500" : toast.type === "warn" ? "bg-amber-500" : "bg-green-600"}`}>
          {toast.type === "error"
      ...
      <div className="pt-14 md:pt-0 md:pl-16 lg:pl-[220px]">
        <main className="px-4 sm:px-6 lg:px-8 py-5 sm:py-7 min-h-screen">
          <div className="flex items-start justify-between mb-5 sm:mb-7">
            <div>
              <h1 className="text-[20px] sm:text-[26px] font-black text-gray-900 tracking-tight">Product Approvals</h1>
              <p className="text-gray-400 text-[12px] sm:text-[13px] mt-0.5">Review pharmacy product submissions</p>
            </div>
            <button onClick={fetchProducts} className="flex items-center gap-2 border border-gray-200 bg-white text-gray-600 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl font-bold text-[12px] sm:text-[13px] hover:border-green-300 hover:text-green-700 transition shadow-sm">
          ...
          {counts.Pending > 0 && (
            <div className="flex flex-wrap items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 sm:px-5 py-3.5 mb-6">
              ...
              <p className="text-[12px] sm:text-[13px] font-semibold text-amber-800"><span className="font-black">{counts.Pending}</span> product{counts.Pending > 1 ? "s" : ""} awaiting approval</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-0 bg-white rounded-2xl border border-gray-100 shadow-sm mb-5 overflow-hidden">
            {[
              ...
              { key: "Pending", label: "Pending", count: counts.Pending, color: "text-amber-600", bg: "bg-amber-50" },
              { key: "Approved", label: "Approved", count: counts.Approved, color: "text-green-600", bg: "bg-green-50" },
              { key: "Rejected", label: "Rejected", count: counts.Rejected, color: "text-red-600", bg: "bg-red-50" },
            ].map((s, i) => (
              <button key={s.key} onClick={() => setFilter(s.key)}
                className={`${s.bg} px-4 sm:px-6 py-4 text-left transition-all ${i % 2 === 0 ? "border-r border-gray-100" : ""} sm:border-r sm:last:border-r-0 border-gray-100 ${i >= 2 ? "border-t sm:border-t-0 border-gray-100" : ""} ${filter === s.key ? "ring-2 ring-inset ring-green-400" : "hover:brightness-95"}`}>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-5">
            <div className="flex gap-1.5 flex-wrap">
              {["All", "Pending", "Approved", "Rejected"].map((key) => (
                <button key={key} onClick={() => setFilter(key)}
                  className={`px-3 py-2 rounded-xl text-[12px] sm:text-[13px] font-medium transition-all flex items-center gap-1.5 ${filter === key ? "bg-gray-950 text-white shadow-sm" : "bg-white text-gray-500 border border-gray-200 hover:border-green-300 hover:text-green-600"}`}>
                  {key}
            ...
            <div className="relative w-full sm:flex-1 sm:min-w-[200px] sm:max-w-sm sm:ml-auto">
          ...
          {loading ? (
            ...
            <>
                ...
                {paginated.map((product) => {
                  ...
                  return (
                    <div key={product._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                      <div className="flex items-start sm:items-center gap-3 px-4 sm:px-5 py-4">
                        <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 mt-1.5 sm:mt-0 ${cfg.dot} ${isPending ? "animate-pulse" : ""}`} />
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-green-50 border border-green-100 flex items-center justify-center text-xl flex-shrink-0 overflow-hidden">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-[11px] text-gray-400 truncate sm:hidden">{pharmacy?.name || "Unknown"}</p>
                            <p className="text-[11px] text-gray-400 truncate hidden sm:block max-w-[200px]">{product.productDescription}</p>
                          </div>
                        </div>
                        <div className="hidden sm:flex flex-1 min-w-0 items-center gap-2">
                          <div className="w-6 h-6 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-white font-black text-[10px] flex-shrink-0">{pharmacy?.name?.[0]?.toUpperCase() || "P"}</div>
                        ...
                        <div className="hidden sm:block flex-shrink-0 text-center">
                          ...
                          <p className="text-[13px] font-black text-gray-900">Rs. {product.productPrice?.toLocaleString()}</p>
                        </div>
                        ...
                        {isPending && (
                          <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
                            <button onClick={(e) => { e.stopPropagation(); handleApproval(product._id, "Approved"); }} disabled={!!acting}
                              className="px-3 py-1.5 text-[11px] font-bold bg-green-500 text-white rounded-lg hover:bg-green-400 transition disabled:opacity-50">
                        ...
                        <button onClick={() => setExpanded(isExpanded ? null : product._id)}
                          ...
                          <svg className={`w-3.5 h-3.5 text-gray-500 transition-transform ${isExpanded ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
                        </button>
                      </div>
                      {isPending && (
                        <div className="sm:hidden flex gap-2 px-4 pb-3">
                          <button onClick={() => handleApproval(product._id, "Approved")} disabled={!!acting}
                            className="flex-1 py-2 text-[12px] font-bold bg-green-500 text-white rounded-xl hover:bg-green-400 transition disabled:opacity-50">✓ Approve</button>
                          <button onClick={() => handleApproval(product._id, "Rejected")} disabled={!!acting}
                            className="flex-1 py-2 text-[12px] font-bold bg-red-50 text-red-600 border border-red-200 rounded-xl hover:bg-red-100 transition disabled:opacity-50">✕ Reject</button>
                        </div>
                      )}
                      {isExpanded && (
                        <ProductDetail product={product} onApprove={(id) => handleApproval(id, "Approved")} onReject={(id) => handleApproval(id, "Rejected")} acting={acting} />
                      )}
                    </div>
                  );
                })}
              ...
              {/* Pagination */}
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm mt-3 px-4">
                <Pagination page={safePage} totalPages={totalPages} onChange={setPage} />
              </div>
              <p className="text-[12px] text-gray-400 mt-3 px-1 text-center">
                Showing {filtered.length === 0 ? 0 : (safePage - 1) * PER_PAGE + 1}–{Math.min(safePage * PER_PAGE, filtered.length)} of {filtered.length} products
              </p>
            </>
          )}
        </main>
