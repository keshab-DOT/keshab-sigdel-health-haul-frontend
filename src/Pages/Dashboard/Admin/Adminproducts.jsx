      ...
      <div className="flex flex-col lg:grid lg:grid-cols-3 gap-5 sm:gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-[15px] sm:text-[16px] font-black text-gray-900 mb-1">
                {product.productName}
              </h3>
              <p className="text-[12px] sm:text-[13px] text-gray-500 leading-relaxed mb-3">
                {product.productDescription || "No description provided."}
              </p>
              <div className="flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-1.5 bg-green-50 border border-green-100 text-green-700 text-[11px] sm:text-[12px] font-bold px-2.5 sm:px-3 py-1 rounded-full">
                  💰 Rs. {product.productPrice?.toLocaleString()}
                </span>
                <span
                  className={`inline-flex items-center gap-1.5 text-[11px] sm:text-[12px] font-bold px-2.5 sm:px-3 py-1 rounded-full border ${product.productTotalStockQuantity > 0 ? "bg-blue-50 border-blue-100 text-blue-700" : "bg-red-50 border-red-100 text-red-600"}`}
                >
                  📦 Stock: {product.productTotalStockQuantity}
                </span>
                {product.categoryId?.categoryName && (
                  <span className="inline-flex items-center gap-1.5 bg-blue-50 border border-blue-100 text-blue-700 text-[11px] sm:text-[12px] font-bold px-2.5 sm:px-3 py-1 rounded-full">
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
            ...
            <div
              className={`mt-1 flex items-center gap-2 px-3 py-2.5 rounded-xl text-[12px] font-bold ${product.approvalStatus === "Approved" ? "bg-green-50 text-green-700 border border-green-100" : "bg-red-50 text-red-600 border border-red-100"}`}
            >
              {product.approvalStatus === "Approved" ? (
                <>
                  <svg
...
export default function AdminProducts() {
  ...
  const handleApproval = async (productId, approvalStatus) => {
    ...
    try {
      ...
      setProducts((p) =>
        p.map((prod) =>
          prod._id === productId ? { ...prod, approvalStatus } : prod,
        ),
      );
      showToast(
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa]">
      {toast && (
        <div
          className={`fixed top-5 right-5 z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-lg text-white text-[13px] font-bold
        ...
        <main className="px-4 sm:px-6 lg:px-8 py-5 sm:py-7 min-h-screen">
          <div className="flex items-start justify-between mb-5 sm:mb-7">
            <div>
              ...
              <p className="text-gray-400 text-[12px] sm:text-[13px] mt-0.5">
                Review pharmacy product submissions
              </p>
            </div>
            <button
          ...
          {counts.Pending > 0 && (
            <div className="flex flex-wrap items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 sm:px-5 py-3.5 mb-6">
              ...
              <p className="text-[12px] sm:text-[13px] font-semibold text-amber-800">
                <span className="font-black">{counts.Pending}</span> product
                {counts.Pending > 1 ? "s" : ""} awaiting approval
              </p>
              <button
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-5">
            <div className="flex gap-1.5 flex-wrap">
              {["All", "Pending", "Approved", "Rejected"].map((key) => (
                ...
                >
                  {key}
                  <span
                    className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${filter === key ? "bg-white/20 text-white" : "bg-gray-100 text-gray-400"}`}
                  >
                    {counts[key]}
                  </span>
                </button>
            ...
            <div className="relative w-full sm:flex-1 sm:min-w-[200px] sm:max-w-sm sm:ml-auto">
              ...
              <input
                type="text"
                placeholder="Search product or pharmacy…"
                value={search}
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-24 text-center">
              ...
              <p className="text-[13px] text-gray-400 mt-1">
                {search
                  ...
                  : `No ${filter === "All" ? "" : filter.toLowerCase()} products`}
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {filtered.map((product) => {
                ...
                const cfg =
                  STATUS_CFG[product.approvalStatus] || STATUS_CFG.Pending;
                const imgSrc = product.productImageUrl
                  ? product.productImageUrl.startsWith("http")
                    ? product.productImageUrl
                    : `https://keshab-sigdel-health-haul-backend-production.up.railway.app/uploads/${product.productImageUrl}`
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start sm:items-center gap-3 px-4 sm:px-5 py-4">
                      <span
                        className={`w-2.5 h-2.5 rounded-full flex-shrink-0 mt-1.5 sm:mt-0 ${cfg.dot} ${isPending ? "animate-pulse" : ""}`}
                      />
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {/* On mobile: show pharmacy name under product name */}
                          <p className="text-[11px] text-gray-400 truncate sm:hidden">
                            {pharmacy?.name || "Unknown"}
                          </p>
                          <p className="text-[11px] text-gray-400 truncate hidden sm:block max-w-[200px]">
                            {product.productDescription}
                          </p>
                        </div>
                      </div>

                      {/* Pharmacy — hidden on mobile */}
                      ...
                      <button
                        onClick={() =>
                          setExpanded(isExpanded ? null : product._id)
                        }
                    ...
                    {isPending && (
                      <div className="sm:hidden flex gap-2 px-4 pb-3">
                        <button
                          ...
                          disabled={!!acting}
                          className="flex-1 py-2 text-[12px] font-bold bg-green-500 text-white rounded-xl hover:bg-green-400 transition disabled:opacity-50"
                        >
                          ✓ Approve
                        </button>
                        <button
                          ...
                          disabled={!!acting}
                          className="flex-1 py-2 text-[12px] font-bold bg-red-50 text-red-600 border border-red-200 rounded-xl hover:bg-red-100 transition disabled:opacity-50"
                        >
                          ✕ Reject
                        </button>
                      </div>
          ...
          {!loading && filtered.length > 0 && (
            <p className="text-[12px] text-gray-400 mt-4 px-1">
